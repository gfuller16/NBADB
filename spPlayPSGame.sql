USE [NBADB]
GO
/****** Object:  StoredProcedure [dbo].[spPlayPSGame]    Script Date: 3/6/2018 8:24:58 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[spPlayPSGame] --2,3
@Team1 INT,
@Team2 INT,
@gmID INT
AS
...
BEGIN
	...
	DECLARE @Team1Seed INT = 
	(SELECT [psSeed] FROM dbo.tblPlayoffSeeding
	WHERE ps_tmID = @Team1)
	...
	DECLARE @Team2Seed INT = 
	(SELECT [psSeed] FROM dbo.tblPlayoffSeeding
	WHERE ps_tmID = @Team2)

	PRINT CAST(@Team1Seed as VARCHAR(1)) + ' ' + @TeamName1 + ' ' + '(' + @Team1Record + ')'
	PRINT 'Vs.'
	PRINT CAST(@Team2Seed as VARCHAR(1)) + ' ' + @TeamName2 + ' ' + '(' + @Team2Record + ')'

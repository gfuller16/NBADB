USE [NBADB]
GO
/****** Object:  StoredProcedure [dbo].[spPlayGame]    Script Date: 4/19/2017 12:40:05 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[spPlayGame] --2,3
@Team1 INT,
@Team2 INT,
@gmID INT
AS
...
BEGIN
	...
	DECLARE @P11pID INT = (SELECT TOP 1 plID FROM dbo.tblPlayer WHERE plTeam = @Team1)
	EXEC spPlayerStats @P11pID,@gmID

	DECLARE @P12pID INT = (SELECT TOP 1 plID FROM dbo.tblPlayer WHERE plTeam = @Team1 AND plID <> @P11pID)
	...
	DECLARE @P21pID INT = (SELECT TOP 1 plID FROM dbo.tblPlayer WHERE plTeam = @Team2)
	EXEC spPlayerStats @P21pID,@gmID

	DECLARE @P22pID INT = (SELECT TOP 1 plID FROM dbo.tblPlayer WHERE plTeam = @Team2 AND plID <> @P21pID)
